...
module.exports = {
  ...
  output: {
    ...
    chunkFilename: 'js/[id].chunk[hash].js' // chunk生成的配置
  },
  module: {
    ...
    loaders: [{
      ...
      loader: 'url-loader?limit=8192&name=./img/asset/[hash].[ext]'
    }]
  },
  plugins: [
    ...
    new HtmlWebpackPlugin({ // 根据模板插入css/js等生成最终HTML
      ...
      template: './src/view/product.html', // html模板路径
      inject: 'body', // js插入的位置，true/'head'/'body'/false

      chunks: ['vendors', 'product'], // 需要引入的chunk，不配置就会引入所有页面的资源
      minify: { // 压缩HTML文件
        removeComments: true, // 移除HTML中的注释
        collapseWhitespace: false // 删除空白符与换行符
      }
    ...
    new HtmlWebpackPlugin({ // 根据模板插入css/js等生成最终HTML
      favicon: './src/img/favicon.ico', // favicon路径，通过webpack引入同时可以生成hash值
      filename: './contact.html', // 生成的html存放路径，相对于path
      template: './src/view/contact.html', // html模板路径
      inject: 'body', // js插入的位置，true/'head'/'body'/false

      chunks: ['vendors', 'contact'], // 需要引入的chunk，不配置就会引入所有页面的资源
      minify: { // 压缩HTML文件
        removeComments: true, // 移除HTML中的注释
        collapseWhitespace: false // 删除空白符与换行符
      }
